...
profi serves `/metrics` over plain HTTP by default (backwards compatible).
...
│   ├── Chart.yaml
│   ├── values.yaml
│   └── templates/          # DaemonSet, RBAC, Service, ServiceMonitor,
│                           # PrometheusRule, cert-manager Certificate,
│                           # Grafana dashboard ConfigMap
└── runbooks/               # Playbooks linked from alert runbook_url
    ├── cardinality-explosion.md
    ├── event-loss.md
    ├── nccl-hang.md
    ├── nccl-straggler.md
    └── metrics-auth-failures.md
```
